...
@Component({
  selector: 'app-exports-manage',
  templateUrl: './exports-manage.component.html'
})
export class ExportsManageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.translate.get('applications.root').subscribe((rootName: string) => {
      this.dataColumns = [
        ...
        { name: 'applications',
          label: this.translate.instant('applications.applications'),
          type: ResultMasterColumnTypes.STRING,
          link: false}
      ];

      this.apiService.getType('export_config')
